...
/* 让球胜平负列表组件 */
export default class ZongjinQiuList extends Component {
  constructor() {
    super(...arguments);
    this.state = {
      chooseSPF: [],
  ...
  componentWillMount() {
    lotteryControl(betConfig.bjdc.jq.gid).then(result => {
      if (result && result.bs) {
        let bs = result.bs
        this.setState({
  ...
  clickSPF(i, type, from = '') {
    ...
    let index = +betConfig.bjdc.jq.location[type];
  ...
  getSpArr(chooseInfo) {
    ...
    if (_.isArray(chooseInfo)) {
      _.each(chooseInfo, item => {
        let tempSp = item.jqs.split(',');
        let tempArr = [];
        let obj = {};
        _.each(item.spfchoose, val => {
          tempArr.push(tempSp[+betConfig.bjdc.jq.location[val]])
        })
    ...
    if (this.props.dataList && this.props.dataList.length > 0) {
      ...
      this.props.dataList.map((item, index) => {
        ...
        if (row && row.length > 0) {
          ...
          arr.push(
            <div key={`zjq_date_${index}`}>
              <div onClick={_this.props.toggleList.bind(this)} data-id={index} data-type="zjq"
                   className="dateDiv down">{addesc + ' ' + rLength + '场比赛'}</div>
              <div id={`zjq_list_${index}`}>
                {
                  (realRow && rLength > 0) &&
                  realRow.map((it, ind) => {
                    i += 1;
                    ...
                    let jqs = it.jqs.split(',');
                    return (
                      <div className="bettingBoxWrap" key={`zjq_${it.itemid}`}>
                        <div className="bettingBoxZongjinqiu clearfix">
                          <div className="bettingBox_fl" onClick={() => this.props.toggle(it.itemid, 'zjq')}>
                            <p>{it.mname}</p>
                            <p>{it.itemid}</p>
                            <p>{`${moment(it.et).format('HH:mm')} 截止`}</p>
                            <p className="arrow " id={`zjq_arrow_${it.itemid}`}/>
                          </div>
                          <div className="bettingBox_fr">
                            ...
                            <table cellSpacing="0" cellPadding="0">
                              <tbody>
                              <tr>
                                <td className={(_this.state.chooseSPF[num] && _this.state.chooseSPF[num][0] === '0') ? 'redRed' : ''}
                                    onClick={
                                      () => {
                                        _this.clickSPF(num, '0')
                                      }}
                                >
                                  <span className="span1">0</span><span className="span2">{jqs[0]}</span>
                                </td>
                                <td className={"border_lr "+((_this.state.chooseSPF[num] && _this.state.chooseSPF[num][1] === '1') ? 'redRed' : '')}
                                    onClick={
                                      () => {
                                        _this.clickSPF(num, '1')
                                      }}
                                >
                                  <span className="span1">1</span><span className="span2">{jqs[1]}</span>
                                </td>
                                <td className={"border_lr2 "+((_this.state.chooseSPF[num] && _this.state.chooseSPF[num][2] === '2') ? 'redRed' : '')}
                                    onClick={
                                      () => {
                                        _this.clickSPF(num, '2')
                                      }}
                                >
                                  <span className="span1">2</span><span className="span2">{jqs[2]}</span>
                                </td>
                                <td className={(_this.state.chooseSPF[num] && _this.state.chooseSPF[num][3] === '3') ? 'redRed' : ''}
                                    onClick={
                                      () => {
                                        _this.clickSPF(num, '3')
                                      }}
                                >
                                  <span className="span1">3</span><span className="span2">{jqs[3]}</span>
                                </td>
                              </tr>


                              <tr>
                                <td className={(_this.state.chooseSPF[num] && _this.state.chooseSPF[num][4] === '4') ? 'redRed' : ''}
                                    onClick={
                                      () => {
                                        _this.clickSPF(num, '4')
                                      }}
                                >
                                  <span className="span1">4</span><span className="span2">{jqs[4]}</span>
                                </td>
                                <td className={"border_lr "+((_this.state.chooseSPF[num] && _this.state.chooseSPF[num][5] === '5') ? 'redRed' : '')}
                                    onClick={
                                      () => {
                                        _this.clickSPF(num, '5')
                                      }}
                                >
                                  <span className="span1">5</span><span className="span2">{jqs[5]}</span>
                                </td>
                                <td className={"border_lr2 "+((_this.state.chooseSPF[num] && _this.state.chooseSPF[num][6] === '6') ? 'redRed' : '')}
                                    onClick={
                                      () => {
                                        _this.clickSPF(num, '6')
                                      }}
                                >
                                  <span className="span1">6</span><span className="span2">{jqs[6]}</span>
                                </td>
                                <td className={(_this.state.chooseSPF[num] && _this.state.chooseSPF[num][7] === '7') ? 'redRed' : ''}
                                    onClick={
                                      () => {
                                        _this.clickSPF(num, '7')
                                      }}
                                >
                                  <span className="span1">7+</span><span className="span2">{jqs[7]}</span>
                                </td>
                              </tr>
                              </tbody>
                            </table>
                        ...
                        return (
                          <div className="mixedProgram " key={`hh_choose_${index}`}>
                            ...
                            <div className="mixedFlex">
                              ...
                              <div className="programBox_fr_shangXia">
                                <table className="bjdcZjq" cellSpacing="0" cellPadding="0">
                                  <tbody>
                                  <tr>
                                    <td className={(_this.state.chooseSPF[index] && _this.state.chooseSPF[index][0] === '0') ? 'redRed' : ''}
                                        onClick={
                                          () => {
                                    ...
                                    <td className={"border_lr "+((_this.state.chooseSPF[index] && _this.state.chooseSPF[index][1] === '1') ? 'redRed' : '')}
                                        onClick={
                                          () => {
                                    ...
                                    <td className={"border_lr2 "+((_this.state.chooseSPF[index] && _this.state.chooseSPF[index][2] === '2') ? 'redRed' : '')}
                                        onClick={
                                          () => {
                                            _this.clickSPF(index, '2','fromPop')
                                          }}
                                    >
                                      <span className="span1">2</span><span className="span2">{jqs[2]}</span>
                                    </td>
                                    <td className={(_this.state.chooseSPF[index] && _this.state.chooseSPF[index][3] === '3') ? 'redRed' : ''}
                                        onClick={
                                          () => {
                                  ...
                                  <tr>
                                    <td className={(_this.state.chooseSPF[index] && _this.state.chooseSPF[index][4] === '4') ? 'redRed' : ''}
                                        onClick={
                                          () => {
                                            _this.clickSPF(index, '4','fromPop')
                                          }}
                                    >
                                      <span className="span1">4</span><span className="span2">{jqs[4]}</span>
                                    </td>
                                    <td className={"border_lr "+((_this.state.chooseSPF[index] && _this.state.chooseSPF[index][5] === '5') ? 'redRed' : '')}
                                        onClick={
                                          () => {
                                            _this.clickSPF(index, '5','fromPop')
                                          }}
                                    >
                                      <span className="span1">5</span><span className="span2">{jqs[5]}</span>
                                    </td>
                                    <td className={"border_lr2 "+((_this.state.chooseSPF[index] && _this.state.chooseSPF[index][6] === '6') ? 'redRed' : '')}
                                        onClick={
                                          () => {
                                            _this.clickSPF(index, '6','fromPop')
                                          }}
                                    >
                                      <span className="span1">6</span><span className="span2">{jqs[6]}</span>
                                    </td>
                                    <td className={(_this.state.chooseSPF[index] && _this.state.chooseSPF[index][7] === '7') ? 'redRed' : ''}
                                        onClick={
                                          () => {
                                            _this.clickSPF(index, '7','fromPop')
                                          }}
                                    >
                                      <span className="span1">7+</span><span className="span2">{jqs[7]}</span>
                                    </td>
