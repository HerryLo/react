...
/* 让球胜平负列表组件 */
export default class BiFenList extends Component {
  constructor() {
    super(...arguments);
    this.state = {
      minBei: '',
  ...
  componentWillMount() {
    lotteryControl(betConfig.bjdc.bf.gid).then(result => {
      if (result && result.bs) {
        let bs = result.bs
        this.setState({
  ...
  checkShow(goTop = false) {
    this.setState({
      ...
      if (goTop) {
        document.getElementById('bdbfdiv').scrollTop = 0
      }
    })
  }
    ...
    if (_.isArray(this.chooseInfo)) {
      ...
      _.each(chooseInfo, item => {
        ...
        _.each(item.chooseWF, (val, index) => {
          if (val) {
            tempArr.push(parseFloat(cbf[+betConfig.bjdc.bf.location[val]]))
          }
        })
        ...
        if (row && row.length > 0) {
          ...
          arr.push(
            <div key={`spf_date_${index}`}>
              <div onClick={_this.props.toggleList.bind(this)} data-id={index} data-type="bf"
                   className="dateDiv down">{addesc + ' ' + rLength + '场比赛'}</div>
              <div id={`bf_list_${index}`}>
                {
                  (realRow && rLength > 0) &&
                  realRow.map((it, ind) => {
                    i += 1;
                    ...
                    return (
                      <div className="bettingBoxWrap" key={`bf_${it.itemid}`}>
                        <div className="bettingBoxSingle">
                          <div className="bettingBox_fl" onClick={() => this.props.toggle(it.itemid, 'bf')}>
                            <p>{it.mname}</p>
                            <p>{it.itemid}</p>
                            <p>{`${moment(it.et).format('HH:mm')}截止`}</p>
                            <p className="arrow " id={`bf_arrow_${it.itemid}`}/>
                          </div>
                            ...
                            <table cellSpacing="0" cellPadding="0">
                              ...
                              <tr>
                                <td className={this.realSPF[num].wfNum > 0 ? "redRed" : "bfBlack"}
                                  ...
                                  {this.realSPF[num].wfNum > 0 ? this.showDesc(this.realSPF[num].chooseWF, 'bf') : "点击选择比分"}
                                </td>
                              </tr>
                              </tbody>
                            </table>
                            ...
                            >
                              {
                                ...
                                <p>
                                  <span className="span3"
                                        onClick={() => this.chooseFromPop(item, item.index)}>{this.showDesc(item.chooseWF, 'bf')}</span>
                                </p>
                              }
                            </div>
                          </div>
      ...
      <div tabIndex="0">
        {
          !this.state.popShow ?
            <div>
              <div className="listDivView" id="bdbfdiv" style={{height: this.props.listHeight}}>
                {this.Template()}
              </div>
