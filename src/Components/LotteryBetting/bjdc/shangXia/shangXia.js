...
/* 让球胜平负列表组件 */
export default class ShangXiaList extends Component {
  constructor() {
    super(...arguments);
    this.state = {
      chooseSPF: [],
  ...
  componentWillMount() {
    lotteryControl(betConfig.bjdc.sxds.gid).then(result => {
      if (result && result.bs) {
        let bs = result.bs
        this.setState({
  ...
  clickSPF(i, type, from = '') {
    ...
    let index = +betConfig.bjdc.sxds.location[type];
  ...
  getSpArr(chooseInfo) {
    ...
    if (_.isArray(chooseInfo)) {
      _.each(chooseInfo, item => {
        let tempSp = item.sxp.split(',');
        let tempArr = [];
        let obj = {};
        _.each(item.spfchoose, val => {
          tempArr.push(tempSp[+betConfig.bjdc.sxds.location[val]])
        })
    ...
    if (this.props.dataList && this.props.dataList.length > 0) {
      ...
      this.props.dataList.map((item, index) => {
        ...
        if (row && row.length > 0) {
          ...
          arr.push(
            <div key={`sxds_date_${index}`}>
              <div onClick={_this.props.toggleList.bind(this)} data-id={index} data-type="sxds"
                   className="dateDiv down">{addesc + ' ' + rLength + '场比赛'}</div>
              <div id={`sxds_list_${index}`}>
                {
                  (realRow && rLength > 0) &&
                  realRow.map((it, ind) => {
                    i += 1;
                    ...
                    let sxp = it.sxp.split(',');
                    return (
                      <div className="bettingBoxWrap" key={`sxds_${it.itemid}`}>
                        <div className="bettingBoxShangxia clearfix">
                          <div className="bettingBox_fl" onClick={() => this.props.toggle(it.itemid, 'sxds')}>
                            <p>{it.mname}</p>
                            <p>{it.itemid}</p>
                            <p>{`${moment(it.et).format('HH:mm')} 截止`}</p>
                            <p className="arrow " id={`sxds_arrow_${it.itemid}`}/>
                          </div>
                          <div className="bettingBox_fr">
                            ...
                            <table cellSpacing="0" cellPadding="0">
                              ...
                              <tr>
                                <td
                                  className={"td1 " + ((_this.state.chooseSPF[num] && _this.state.chooseSPF[num][3] === '3') ? 'redRed' : '')}
                                  onClick={
                                    () => {
                                      _this.clickSPF(num, '3')
                                    }}
                                >
                                  <p>上单</p><p className="p1">{sxp[3]}</p>
                                </td>
                                <td
                                  className={"td1 border_lr " + ((_this.state.chooseSPF[num] && _this.state.chooseSPF[num][2] === '2') ? 'redRed' : '')}
                                  onClick={
                                    () => {
                                      _this.clickSPF(num, '2')
                                    }}
                                >
                                  <p>上双</p><p className="p1">{sxp[2]}</p>
                                </td>
                                <td
                                  className={"td1 border_lr2 " + ((_this.state.chooseSPF[num] && _this.state.chooseSPF[num][1] === '1') ? 'redRed' : '')}
                                  onClick={
                                    () => {
                                      _this.clickSPF(num, '1')
                                    }}
                                >
                                  <p>下单</p><p className="p1">{sxp[1]}</p>
                                </td>
                                <td
                                  className={"td1 " + ((_this.state.chooseSPF[num] && _this.state.chooseSPF[num][0] === '0') ? 'redRed' : '')}
                                  onClick={
                                    () => {
                                      _this.clickSPF(num, '0')
                                    }}
                                >
                                  <p>下双</p><p className="p1">{sxp[0]}</p>
                                </td>
                              </tr>
                              </tbody>
                            </table>
                          ...
                          <div className="mixedProgram " key={`hh_choose_${index}`}>
                            ...
                            <div className="mixedFlex">
                              ...
                              <div className="programBox_fr_shangXia">
                                <table className="sxds" cellSpacing="0" cellPadding="0">
                                  <tbody>
                                  <tr>
                                    <td
                                      className={"td1 " + ((_this.state.chooseSPF[index] && _this.state.chooseSPF[index][3] === '3') ? 'redRed' : '')}
                                      onClick={
                                        () => {
                                    ...
                                    <td
                                      className={"td1 border_lr " + ((_this.state.chooseSPF[index] && _this.state.chooseSPF[index][2] === '2') ? 'redRed' : '')}
                                      onClick={
                                        () => {
                                    ...
                                    <td
                                      className={"td1 border_lr2 " + ((_this.state.chooseSPF[index] && _this.state.chooseSPF[index][1] === '1') ? 'redRed' : '')}
                                      onClick={
                                        () => {
                                    ...
                                    <td
                                      className={"td1 " + ((_this.state.chooseSPF[index] && _this.state.chooseSPF[index][0] === '0') ? 'redRed' : '')}
                                      onClick={
                                        () => {
