...
export class Z3DS extends Component {
  constructor() {
    super(...arguments)
    this.state = {
      yilouShow: false,
  ...
  clickItem(e) {
    ...
    if (child.indexOf(val) >= 0) {
      ...
      console.log(wei)
      if (wei === 'm0') {
        if (choose.m1.indexOf(val) >= 0) {
          choose.m1 = choose.m1.slice(0, choose.m1.indexOf(val)).concat(choose.m1.slice(choose.m1.indexOf(val) + 1))
        }
      } else {
        if (choose.m0.indexOf(val) >= 0) {
          choose.m0 = choose.m0.slice(0, choose.m0.indexOf(val)).concat(choose.m0.slice(choose.m0.indexOf(val) + 1))
        }
      }
    }
    console.log(choose)
    ...
    let zhushu = m0.length * m1.length
    this.setState({
      choose: choose,
      zhushu: zhushu,
      delShow: delShow
  ...
  template(from, to, wei, desc) {
    ...
    arr.push(
      <div className="column" key={"chooseList" + wei}>
        {desc ?
    ...
    return (
      <div>
        <div className="listDivView" style={{height: this.props.listHeight || 350}}>
          <div className="fc3DBetting">
            ...
            <div className="redBallTitle">
              ...
              <p className="p1">猜中开奖号码即中奖<span>346</span>元</p>
              <ZhuShou receiveYL={this.receiveYL}
                       yilouShow={this.state.yilouShow}
                       hasYL="no"
        ...
        <Pl3Foot1 type={this.type}
                  ...
                  doNext={() => {
                    let choose = this.state.choose
                    if (choose.m0.length === 0 && choose.m1.length === 0) {
                      this.creatOne()
                      return;
                    }
