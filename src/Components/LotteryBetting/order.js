...
class Order extends Component {
  ...
  getDesc(CG, gid) {
    let str = ''
    if (gid === '80' || gid === '81' || gid === '98' || gid === '99') {
      return str
    } else {
      if (CG) {
        _.each(CG, item => {
  ...
  doBuy() {
    ...
    if (this.state.data.yczs) {
      ...
      if (data.gid === '98' || data.gid === '99') {
        params.imoneyrange = +data.bonus.max;
        params.iminrange = +data.bonus.min
      } else {
        params.imoneyrange = (+data.bonus.max * data.beiNum).toFixed(2);
        params.iminrange = (+data.bonus.min * data.beiNum).toFixed(2)
      }
