...
class SsqAll extends Component{
  constructor(props){
    super(props)
    this.state ={
      initData: [{
        ...
        >
          {
            initData.map((item, index)=>{
              ...
              return (
                <a key={item.pid} onClick={()=> {this.ClickHashGoto(item.pid)}} className="lotteryA">
                  <p className="p1">
                  ...
                  <p className="p2">
                    ...
                    {
                      blueBall.map((it, index)=> {
                        return (
                          judge?<span key={index} className="ballSpan ballBlue">{it}</span>:
                          <span key={index} className="ballSpan blueColor">{it}</span>
                        )
                      })
                    }
                  </p>
