...
class Yunduojin extends Component {
  constructor() {
    super(...arguments)
    this.state = {
      list: [],
  ...
  render() {
    ...
    function template() {
      ...
      _this.state.list.map((item, itNo) => {
        ...
        arr.push(
          <a className="lotteryA arrowNo" key={`result${itNo}`}>
            <p className="p1">
            ...
            <p className="p2">
              {code.map((item, index) => {
                return (
                  <span key={`code${index}`}
                        className={'ballSpan ' + (itNo === 0 ? "ballRed" : 'redColor')}>{item}</span>
                )
              })}
            </p>
          </a>
