...
import {sendSMS, verifySms, relatedRegister} from '../../Stubs/API'
...
class RegisterTel extends Component {
  ...
  register() {
    ...
    if (userPassCheck(userName, password)) {
      ...
      relatedRegister(params).then(result => {
        if (result.code !== '0') {
          Toast.info(result.desc, 1, null, false)
        } else {
        ...
        <div style={{display: this.state.phoneIsOk ? '' : 'none'}}>
          ...
          <div className="padd_Left30">
            ...
            <div className="padd_right30">
              <p className="tipsP" style={{textAlign:'left',marginTop:'1.8rem',marginBottom:'0.6rem'}}>关联后你可以通过微信账号直接登录</p>                                                     
              <div onClick={this.state.registerLight ? this.register : ""}
                   className={this.state.registerLight ? "loginBtn bgOrange" : "loginBtn"} style={{marginTop:0}}>
                   立即注册
              </div>
            </div>
          </div>
