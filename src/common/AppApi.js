...
import {UserIndexAPI, authorizeToCL} from '../Stubs/API'
import {saveUserName} from '../Components/Login/loginUtils'
...
export const AppJiek = {
  ...
  async thirdAppLoginCheck(fn, isflag) {
    ...
    } else if (!flag && browser.versions._weixin) {
      location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxec3eaef9e277c9bb&redirect_uri=https%3A%2F%2Ft2015.9188.com%2Fwechat%2Fweblogin.go&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';
...
export const ChelunApp = {
  /* 车轮appid
   * 测试 appid: dde7338db65d796eece576f5e69c5194
   * 正式 appid: fe050ade204fdf8e44c612c95806311a
   * */
  chelunappid: 'fe050ade204fdf8e44c612c95806311a',
    ...
    chelunJSBridge.invoke(ChelunApp.chelunappid, (res) => {
      if (res.code == 1) {
        ...
        if (data.openID && data.accessToken) {
          ChelunApp.sendOpenid_Token(data);
        } else {
          Toast.info('获取授权失败,请稍后重试', 2, null, false);
        }
      }
    })
  },
  ...
  sendOpenid_Token(data) {
    ...
    authorizeToCL(params).then((res) => {
      if (res.code == '0') {
        ...
        utils.showAlert('温馨提示', '该手机号绑定过多,请直接登录', '确定', () => {
